...
def get_args() -> argparse.Namespace:
    ...
    arg_parser.add_argument("--input", "-i", help="path to input file", type=str)
    arg_parser.add_argument(
        "--write_dir", "-wd", help="path to write outputs to", type=str
    )
    arg_parser.add_argument(
        "--timeout",
        "-to",
    ...
    arg_parser.add_argument(
        "--log_level", "-l", help="log level", type=str, default="INFO"
    )
    arg_parser.add_argument(
        "--subset_size",
        "-ss",
    ...
    arg_parser.add_argument(
        "--num_batch", "-nb", help="number of docs per work batch", type=int, default=10
    )
    arg_parser.add_argument(
        "--single_url_debug",
        "-sud",
    ...
    arg_parser.add_argument("--num_workers", "-nw", type=int, default=None)
...
class URLBatchProvider(mp.Process):
    ...
    def _url_iterator(self):  # noqa
        ...
        url_list = [
            (u, h)
            for u, h in zip(urls_df["url"].tolist(), urls_df["url_hash"].tolist())
        ]
...
def main():
    ...
    provider_process = URLBatchProvider(
        inputs_queue,
        args.input,
