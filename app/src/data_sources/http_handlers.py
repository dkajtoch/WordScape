...
def header_handler(
    ...
    @param valid_content_type_regex_pattern: Valid content type regex pattern
    ...
    if response.status_code != 200:
        return (response, header_metadata, HTTPError(status_code=response.status_code))

    header_metadata = {
        k: response.headers.get(k, None) for k in settings.download.HEADER_FIELDS
    }

    # check for valid content length
    content_length, exception = valid_content_length(header_metadata["content-length"])
    ...
    content_type, exception = valid_content_type(
        header_metadata["content-type"], valid_content_type_regex_pattern
    )
...
def body_handler(response: requests.Response, exception: Exception) -> Tuple[
    Union[requests.Response, None],
    dict,
    ...
    if response.status_code != 200:
        return (response, body_metadata, HTTPError(status_code=response.status_code))
